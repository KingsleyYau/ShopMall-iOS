...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    ...
    if ([response isKindOfClass:[NSHTTPURLResponse class]]){
        ...
        self.contentType = [[[dict objectForKey:@"Content-Type"] copy] autorelease];
        if ([self.delegate respondsToSelector:@selector(connection:startDownload:)]) {
            if (![self.delegate connection:self startDownload:self.contentType]) {
                // 不允许下载
                [self stopDownload];
            }
        }
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {	// download's done, so do something with the data
    if (connection == self.urlConnection) {
        isConnected = false;
        if ([self.delegate respondsToSelector:@selector(connection:handleData:)]) {
            [self.delegate connection:self handleData:tempData];
        }
