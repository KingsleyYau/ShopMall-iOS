...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.scrollView = [[UIScrollView alloc] initWithFrame:selfFrame];
        ...
        self.contentView = [[UIView alloc] initWithFrame:selfFrame];
        self.contentView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleTopMargin;
...
- (void)layoutSubviews {
    ...
    UIImage *backgroundImage = [UIImage imageWithContentsOfFile:[ResourceManager resourceFilePath:NavScrollTabBackgroundOpaque]];
    ...
    if (self.contentView.frame.size.width > self.frame.size.width + 10) {
        if (!self.leftScrollButton) {
            UIImage *leftScrollImage = [UIImage imageWithContentsOfFile:[ResourceManager resourceFilePath:NavScrollTabLeftEndCap]];
        ...
        if (!self.rightScrollButton) {
            UIImage *rightScrollImage = [UIImage imageWithContentsOfFile:[ResourceManager resourceFilePath:NavScrollTabRightEndCap]];
...
- (void)addButton:(UIButton *)aButton shouldHighlight:(BOOL)shouldHighlight {
	...
	UIImage *stretchableButtonImage = [[UIImage imageWithContentsOfFile:[ResourceManager resourceFilePath:NavScrollTabSelectedTab]] stretchableImageWithLeftCapWidth:15 topCapHeight:0];
    ...
    if ([aButton titleForState:UIControlStateNormal] != nil) {
        ...
        aButton.titleLabel.font = [UIFont boldSystemFontOfSize:BUTTON_FONT_SIZE];
...
- (void)setDefaultButtonPressed:(id)sender
{
    ...
    if (pressedButton.adjustsImageWhenHighlighted 
        ...
        UIImage *buttonImage = [UIImage imageWithContentsOfFile:[ResourceManager resourceFilePath:NavScrollTabSelectedTab]];
...
{
    [self setDefaultButtonPressed:sender];
