...
@implementation ShopPersonalNews(Custom)
+ (NSNumber *)idWithDict:(NSDictionary *)dict {
    NSNumber *numberID = nil;
    id foundValue = [dict objectForKey:ShopPersonalNewsID];
...
- (void)updateWithDict:(NSDictionary *)dict {
    id foundValue = nil;
    // 资讯id
    foundValue = [dict objectForKey:ShopNewsID];
    if(nil != foundValue && [NSNull null] != foundValue && [foundValue isKindOfClass:[NSNumber class]]) {
        self.shopNewsID = foundValue;
    }
    // 生效开始时间
    ...
    // 使用时间
    foundValue = [dict objectForKey:ShopPersonalNewsUseTime];
    if(nil != foundValue && [NSNull null] != foundValue && [foundValue isKindOfClass:[NSNumber class]]) {
        self.useDate = [NSDate dateWithTimeIntervalSince1970:[foundValue intValue]];
    }
    // 获得时间
    foundValue = [dict objectForKey:ShopPersonalNewsGetTime];
    if(nil != foundValue && [NSNull null] != foundValue && [foundValue isKindOfClass:[NSNumber class]]) {
        self.getDate = [NSDate dateWithTimeIntervalSince1970:[foundValue intValue]];
    }
    // 取消时间
    foundValue = [dict objectForKey:ShopPersonalNewsCancelTime];
    if(nil != foundValue && [NSNull null] != foundValue && [foundValue isKindOfClass:[NSNumber class]]) {
        self.getDate = [NSDate dateWithTimeIntervalSince1970:[foundValue intValue]];
    }
    // 标题
    ...
    // getTips
    foundValue = [dict objectForKey:ShopNewsBuyTips];
    if(nil != foundValue && [NSNull null] != foundValue && [foundValue isKindOfClass:[NSString class]]) {
        self.buyTips = foundValue;
    }
    // status
    foundValue = [dict objectForKey:ShopPersonalNewsStatus];
    if(nil != foundValue && [NSNull null] != foundValue && [foundValue isKindOfClass:[NSNumber class]]) {
        self.status = foundValue;
    }
    
    // 条形码
    foundValue = [dict objectForKey:ShopPersonalNewsVericode];
    if(nil != foundValue && [NSNull null] != foundValue && [foundValue isKindOfClass:[NSString class]]) {
        self.vericode = foundValue;
    }
    // 二维码
    foundValue = [dict objectForKey:ShopPersonalNewsUrl];
    if(nil != foundValue && [NSNull null] != foundValue && [foundValue isKindOfClass:[NSString class]]) {
