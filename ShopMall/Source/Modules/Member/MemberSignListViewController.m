//
//  MemberSignListViewController.m
//  ShopMall
//
...
@interface MemberSignListViewController () <ShopRequestOperatorDelegate, EGORefreshTableHeaderDelegate> {
...
@implementation MemberSignListViewController
...
- (void)setupNavigationBar {
    ...
    if(_totalItems > 0) {
        title = [NSString stringWithFormat:@"我的签到(共%d条)", _totalItems];
    }
...
- (void)reloadData:(BOOL)isReloadView {
    NSArray *array = [ShopDataManager signWithUserCurrent];
    if(array) {
...
- (BOOL)loadFromServer:(BOOL)loadMore{
    ...
    if(loadMore) {
        // 加载更多
        return [self.requestOperator updateMemberSignList:_maxItem];
    }
    else {
        // 刷最新
        return [self.requestOperator updateMemberSignList:0];
    }
}
...
- (void)requestFail:(NSString*)error requestType:(ShopRequestOperatorStatus)type {
    ...
    switch(type){
        case ShopRequestOperatorStatus_UpdateMemberSignList:{
            [self setTopStatusText:@"获取我的签到列表失败"];
            break;
        }
        default:break;
    }
