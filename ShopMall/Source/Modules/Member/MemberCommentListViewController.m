//
//  MemberCommentListViewController.m
//  ShopMall
//
...
@interface MemberCommentListViewController () <ShopRequestOperatorDelegate, EGORefreshTableHeaderDelegate> {
...
@implementation MemberCommentListViewController
...
- (void)setupNavigationBar {
    ...
    if(_totalItems > 0) {
        title = [NSString stringWithFormat:@"我的点评(共%d条)", _totalItems];
    }
...
- (void)reloadData:(BOOL)isReloadView {
    NSArray *array = [ShopDataManager CommentWithUserCurrent];
    if(array) {
...
- (BOOL)loadFromServer:(BOOL)loadMore{
    ...
    if(loadMore) {
        // 加载更多
        return [self.requestOperator updateMemberCommentList:_maxItem];
    }
    else {
        // 刷最新
        return [self.requestOperator updateMemberCommentList:0];
    }
}
...
- (void)requestFail:(NSString*)error requestType:(ShopRequestOperatorStatus)type {
    ...
    switch(type){
        case ShopRequestOperatorStatus_UpdateMembeCommentList:{
            [self setTopStatusText:@"获取我的点评列表失败"];
            break;
        }
        default:break;
    }
