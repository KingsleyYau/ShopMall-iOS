...
@interface ShopPhotoListViewController () <ShopRequestOperatorDelegate, EGORefreshTableHeaderDelegate, UIActionSheetDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate> {
...
@implementation ShopPhotoListViewController
...
- (void)setupNavigationBar {
    ...
    if(_totalItems > 0) {
        title = [NSString stringWithFormat:@"商户图片(共%d条)", _totalItems];
    }
...
- (void)reloadData:(BOOL)isReloadView {
    NSArray *array = [ShopDataManager shopImageWithShopID:self.item.shopID imageType:nil];
    if(array) {
...
- (BOOL)loadFromServer:(BOOL)loadMore{
    ...
    if(loadMore) {
        // 加载更多
        return [self.requestOperator updateShopPicture:self.item.shopID curMaxRow:_maxItem];
    }
    else {
        // 刷最新
        return [self.requestOperator updateShopPicture:self.item.shopID curMaxRow:0];
    }
}
