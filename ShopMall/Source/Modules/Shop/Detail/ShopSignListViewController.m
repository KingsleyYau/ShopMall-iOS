...
@interface ShopSignListViewController () <ShopRequestOperatorDelegate, EGORefreshTableHeaderDelegate> {
...
@implementation ShopSignListViewController
...
- (void)setupNavigationBar {
    ...
    if(_totalItems > 0) {
        title = [NSString stringWithFormat:@"商户签到(共%d条)", _totalItems];
    }
...
- (void)reloadData:(BOOL)isReloadView {
    NSArray *array = [ShopDataManager signWithShopID:self.item.shopID];
    if(array) {
...
- (BOOL)loadFromServer:(BOOL)loadMore{
    ...
    if(loadMore) {
        // 加载更多
        return [self.requestOperator updateShopSign:self.item.shopID curMaxRow:_maxItem];
    }
    else {
        // 刷最新
        return [self.requestOperator updateShopSign:self.item.shopID curMaxRow:0];
    }
}
